...
try:
    ...
    w, h = profile.size
    target_ratio = half_width / FULL_HEIGHT
    img_ratio = w / h
    ...
    profile = profile.resize((half_width, FULL_HEIGHT), Image.Resampling.LANCZOS)
...
try:
    ...
    logo_target_w = half_width - 60
    logo.thumbnail((logo_target_w, FULL_HEIGHT), Image.Resampling.LANCZOS)
    logo_w, logo_h = logo.size

    logo_x = half_width + (half_width - logo_w) // 2
    logo_y = 30
    front_img.paste(logo, (logo_x, logo_y))
    content_start_y = logo_y + logo_h + 10
except Exception as e:
...
y_offset = content_start_y + 10
draw_front.text(((half_width + (half_width - get_text_width(name_text, font_large)) // 2), y_offset), name_text, font=font_large, fill=GOLD)
y_offset += 60
draw_front.text(((half_width + (half_width - get_text_width(title_text, font_medium)) // 2), y_offset), title_text, font=font_medium, fill=LIGHT_GRAY)

y_offset += 80
# Contact info left-aligned within a block
contact_block_x = half_width + 40
